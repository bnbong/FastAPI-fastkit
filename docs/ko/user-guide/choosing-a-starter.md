...
FastAPI-fastkit은 프로젝트를 시작하는 여러 가지 방법을 제공합니다. 이 페이지는 처음 쓰는 분을 위한 **선택 가이드**입니다. 여기서 방향을 정했다면 실제 프로젝트 생성은 [퀵 스타트](quick-start.md)에서 이어가세요.

아직 정하지 못했다면 답은 하나입니다.

> **`fastkit init --interactive`로 시작해서 `domain-starter` 프리셋을 선택하세요.** 현재 권장 기본값입니다.

아래에서는 그 이유와 다른 선택이 나은 경우를 설명합니다.
...
이 둘이 메인 진입점이고 용도가 서로 다릅니다.

### `fastkit startdemo <template>`

기본 제공 템플릿(`fastapi-default`, `fastapi-async-crud`, `fastapi-psql-orm`, `fastapi-domain-starter`, ...) 가운데 하나를 바탕으로 **바로 실행해 볼 수 있는 완성형 예제 프로젝트**를 디스크에 만듭니다. 템플릿 소스 코드는 거의 그대로 복사하고, 메타데이터 플레이스홀더(`<project_name>` 등)만 치환합니다.
...
**대화형 마법사**를 따라 진행합니다. 프로젝트 메타데이터 → 아키텍처 프리셋 → 기능 선택(데이터베이스, 인증, 테스트, 배포, ...) → 패키지 매니저 → 확인 순서로 이어집니다. 생성기가 프리셋별로 알맞은 베이스 템플릿을 고른 뒤 그 위에 선택한 기능을 덧입힙니다.
...
이 프리셋들은 `fastkit init --interactive` 의 프로젝트 정보 입력 다음 단계에서 나타납니다. 어떤 프리셋을 고를지 정할 때 이 절을 참고하세요.

### `minimal` — 가장 단순하게 시작, 나중에 키우기

가장 작은 동작 가능한 FastAPI 앱. 빈 스캐폴드에 선택한 기능 플래그를 얹어 다시 만들어 낸 단일 `src/main.py` 입니다. CORS, 레이트 제한, Prometheus 계측은 선택 시 자동으로 `main.py` 에 연결됩니다.
...
모든 코드가 한 모듈 안에 들어갑니다. `minimal` 과 마찬가지로 `main.py` 가 재생성되는 오버레이를 사용합니다.
...
"Django 풍" 레이아웃 — 코드를 관심사별로 수평 분할합니다: 라우터는 모두 `api/`, CRUD 로직은 모두 `crud/`, pydantic 스키마는 모두 `schemas/`, 설정은 모두 `core/`. 기본 제공된 `main.py` 는 **보존**됩니다 (이미 CORS가 연결되어 있습니다). 생성된 데이터베이스/인증 설정 파일은 `src/core/` 아래에 놓입니다.
...
코드를 **비즈니스 개념별**로 수직 분할합니다: 각 도메인은 자신의 router, service, repository, schemas 를 `src/app/domains/<concept>/` 아래에 소유합니다. `/health` 엔드포인트와, 새 개념마다 복사해 이름만 바꾸면 되는 `items` 예제 도메인이 함께 제공됩니다. 기본 제공된 `main.py` (`src/app/` 아래) 는 보존되고, 생성된 설정 파일은 `src/app/core/` 아래에 놓입니다.
...
`fastkit startdemo <template>`는 기능을 직접 조합하는 방식보다 **완성된 실행 예제**가 필요할 때 가장 잘 맞습니다. 대부분의 템플릿은 위 네 가지 프리셋 중 하나와 대략 대응하되, 목 스토어 기반 CRUD 엔드포인트나 응답 포매팅 예제, Docker 도구 같은 추가 예제 코드까지 함께 담고 있습니다.
...
`fastkit list-templates` 를 실행하면 한 줄 설명과 함께 현재 템플릿 목록이 나옵니다.

## 자주 묻는 질문

**Q. 프리셋 / 템플릿을 미리 결정해야 하나요?**
아닙니다 — 생성된 코드는 나중에 손으로 얼마든지 재구성합니다. 프리셋은 출발점일 뿐 계약이 아니니 너무 깊게 고민하지 마세요.

**Q. 지금 추천하는 기본 선택은 무엇인가요?**
`domain-starter`입니다. pyproject를 우선으로 쓰고 `/health` 엔드포인트가 기본으로 들어 있으며, 관리하기 쉬운 중간 규모 FastAPI 프로젝트 구조를 자연스럽게 따라갑니다.

**Q. 나중에 `classic-layered` 에서 `domain-starter` 로 바꿀 수 있나요?**
네, 다만 마이그레이션 명령이 없어 리팩터링을 직접 해야 합니다. 프로젝트가 자라 도메인 폴더가 필요해질 것 같으면 처음부터 `domain-starter` 로 시작하세요.

**Q. 그냥 FastAPI를 학습하고 싶다면요?**
`fastkit startdemo fastapi-default`로 시작하세요. 코드를 읽고, 테스트를 실행하고, 엔드포인트 몇 개를 직접 바꿔 보세요. 익숙해지면 `fastkit init --interactive`의 `domain-starter` 프리셋으로 넘어가면 됩니다.

**Q. 각 프리셋이 정확히 어떤 파일을 생성하는지는 어디서 보나요?**
[아키텍처 프리셋 매트릭스](../reference/preset-feature-matrix.md) 가 그 레퍼런스입니다.
