...
*스냅샷 검증 시점: 2026-05-07. Phase 3(contributing + reference) 작업이 반영된 현재 브랜치 기준으로 `ko` 행을 다시 집계했습니다. 한국어는 언어별 페이지가 모두 존재하며, `docs/ko/changelog.md` 는 영문 기준 changelog를 그대로 가리킵니다.* 이 표는 수동으로 관리됩니다. 리포지토리 루트에서 현재 상태를 다시 세고 싶다면 다음 명령을 실행하세요:
