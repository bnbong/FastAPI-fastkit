...
| 🇯🇵 Japanese (`ja`) | 🟡 Partial | 7 / 26 | Phase 1 landed: top-level + core user-guide + `reference/translation-status.md`. Remaining pages fall back to English. |
...
*Snapshot verified 2026-05-10; ja row recounted for the current branch after Phase 1 (top-level + core user-guide + `reference/translation-status.md`) landed.* These counts are maintained by hand;
